...
public class EmpresaBean implements Serializable {
	...
	private Integer mostrarDireccionesNit;
	...
	public void cargarEmpresas() {
		try {
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", "No se pudieron cargar las empresas."));
		}
	}
		...
		try {
			...
			if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
				...
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Error", "No se pudo crear la empresa. Código: " + resp.statusCode()));
			}
	...
	public void actualizarEmpresa() {
		try {
			if (empresaSeleccionada == null || empresaSeleccionada.getNit() == null)
				return;

			empresaSeleccionada.setDireccionId(null);

			String json = gson.toJson(empresaSeleccionada);
			HttpRequest req = HttpRequest.newBuilder().uri(URI.create(BASE_URL + "/" + empresaSeleccionada.getNit()))
					.header("Content-Type", "application/json").PUT(HttpRequest.BodyPublishers.ofString(json)).build();

			HttpResponse<String> resp = httpClient.send(req, HttpResponse.BodyHandlers.ofString());
			if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_INFO, "Éxito", "Empresa actualizada"));
			} else {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Error", "No se pudo actualizar. Código: " + resp.statusCode()));
			}
			cargarEmpresas();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void cargarDireccionesPorEmpresa(Integer nit) {
		try {
			HttpRequest req = HttpRequest.newBuilder().GET().uri(URI.create(DIR_BY_EMPRESA + "/" + nit))
					.header("Content-Type", "application/json").build();
			HttpResponse<String> resp = httpClient.send(req, HttpResponse.BodyHandlers.ofString());
			...
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error",
					"No se pudieron cargar las direcciones de la empresa."));
		}
	}
	...
	public void crearDireccionParaEmpresa() {
		try {
			...
			HttpRequest req = HttpRequest.newBuilder().uri(URI.create("http://localhost:8083/api/direcciones"))
					.header("Content-Type", "application/json").POST(HttpRequest.BodyPublishers.ofString(json)).build();
			HttpResponse<String> resp = httpClient.send(req, HttpResponse.BodyHandlers.ofString());
			if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_INFO, "Éxito", "Dirección creada y asociada"));
			} else {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Error", "No se pudo crear la dirección. Código: " + resp.statusCode()));
			}
			nuevaDireccionParaEmpresa = new DireccionEmpresaDTO();
			cargarEmpresas();
			if (mostrarDireccionesNit != null)
				cargarDireccionesPorEmpresa(mostrarDireccionesNit);
		} catch (Exception e) {
			e.printStackTrace();
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_FATAL, "Error", e.getMessage()));
	...
	public void prepararEliminar(Integer nit) {
		...
		FacesContext.getCurrentInstance().addMessage(null,
				new FacesMessage(FacesMessage.SEVERITY_INFO, "Preparado", "Listo para poner inactivo NIT: " + nit));
	}
	...
	public void eliminarEmpresa() {
		try {
			...
			HttpRequest req = HttpRequest.newBuilder().uri(URI.create(BASE_URL + "/" + eliminarNit))
					.header("Content-Type", "application/json").PUT(HttpRequest.BodyPublishers.ofString(json)).build();
			HttpResponse<String> resp = httpClient.send(req, HttpResponse.BodyHandlers.ofString());
			if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_INFO, "Hecho", "Empresa marcada como inactiva"));
			} else {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Error", "No se pudo marcar inactiva. Código: " + resp.statusCode()));
			}
			cargarEmpresas();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<DireccionEmpresaDTO> getDireccionesEmpresaSeleccionada() {
		return direccionesEmpresaSeleccionada;
	}

	public void setDireccionesEmpresaSeleccionada(List<DireccionEmpresaDTO> direccionesEmpresaSeleccionada) {
		this.direccionesEmpresaSeleccionada = direccionesEmpresaSeleccionada;
	}

	public DireccionEmpresaDTO getNuevaDireccionParaEmpresa() {
		return nuevaDireccionParaEmpresa;
	}

	public void setNuevaDireccionParaEmpresa(DireccionEmpresaDTO nuevaDireccionParaEmpresa) {
		this.nuevaDireccionParaEmpresa = nuevaDireccionParaEmpresa;
	}

	public Integer getEliminarNit() {
		return eliminarNit;
	}

	public void setEliminarNit(Integer eliminarNit) {
		this.eliminarNit = eliminarNit;
	}

	public Integer getMostrarDireccionesNit() {
		return mostrarDireccionesNit;
	}

	public void setMostrarDireccionesNit(Integer mostrarDireccionesNit) {
		this.mostrarDireccionesNit = mostrarDireccionesNit;
	}
}
