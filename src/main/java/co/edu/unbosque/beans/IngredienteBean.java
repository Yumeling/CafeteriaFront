...
public class IngredienteBean implements Serializable {
		...
		try {
			...
			if (json.startsWith("[")) {
				...
				if (je.getAsJsonObject().has("data")) {
					ingredientes = Arrays
							.asList(gson.fromJson(je.getAsJsonObject().get("data"), IngredienteDTO[].class));
				} else {
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", "No se pudieron cargar los ingredientes."));
		}
	}
		...
		try {
			...
			if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
				...
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Error", "No se pudo crear (código: " + resp.statusCode() + ")."));
			}
	...
	public void actualizarIngrediente() {
		try {
			if (ingredienteSeleccionado == null || ingredienteSeleccionado.getCodigo() == null) {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Error", "Seleccione un ingrediente para editar."));
				return;
			}
	...
	public void prepararEliminar(Integer codigo) {
		...
		FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Preparado",
				"Listo para marcar como agotado (código: " + codigo + ")."));
	}

	public void eliminarIngrediente() {
		try {
			if (eliminarCodigo == null)
				return;
			...
			HttpRequest req = HttpRequest.newBuilder().uri(URI.create(BASE_URL + "/" + eliminarCodigo))
					.header("Content-Type", "application/json").PUT(HttpRequest.BodyPublishers.ofString(json)).build();
			...
			if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
				...
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Error", "No se pudo marcar como agotado (código: " + resp.statusCode() + ")."));
			}
	...
	public Integer getEliminarCodigo() {
		return eliminarCodigo;
	}

	public void setEliminarCodigo(Integer eliminarCodigo) {
