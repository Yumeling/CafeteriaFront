...
public class OrdenCompraBean implements Serializable {
	...
	public void cargarOrdenes() {
		try {
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", "No se pudieron cargar las órdenes."));
		}
	}
	...
	public void addIngredienteToNuevaOrden() {
		try {
			...
			if (nuevaOrden.getIngredientes() == null)
				nuevaOrden.setIngredientes(new ArrayList<>());
	...
	public void removeIngredienteFromNuevaOrden(Integer codigo) {
		if (codigo == null || nuevaOrden.getIngredientes() == null)
			return;
		...
		try {
			...
			if (nuevaOrden.getIngredientes() == null || nuevaOrden.getIngredientes().isEmpty()) {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Error", "Agrega al menos un ingrediente a la orden."));
				return;
			}
			...
			if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_INFO, "Éxito", "Orden creada"));
			} else {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Error", "No se pudo crear la orden. Código: " + resp.statusCode()));
			}
	...
	public void actualizarOrden() {
		try {
			if (ordenSeleccionada == null || ordenSeleccionada.getOrdenId() == null)
				return;
			...
			if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_INFO, "Éxito", "Orden actualizada"));
			} else {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
	...
	public void prepararCancelar(Integer id) {
		...
		FacesContext.getCurrentInstance().addMessage(null,
				new FacesMessage(FacesMessage.SEVERITY_INFO, "Preparado", "Listo para inactivar la orden id: " + id));
	}
	...
	public void eliminarOrden() {
		try {
			...
			if (id == null) {
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", "ID inválido para inactivar."));
				return;
			}
			...
			if (target == null) {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Error", "No se encontró la orden para inactivar."));
				return;
			}
			...
			if (status >= 200 && status < 300) {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Hecho",
						"Orden marcada como inactiva (id: " + id + ")."));
			} else {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Error", "No se pudo inactivar la orden. Código: " + status + " Respuesta: " + body));
			}
	...
	public List<IngredienteDTO> getAvailableIngredientes() {
		return availableIngredientes;
	}

	public void setAvailableIngredientes(List<IngredienteDTO> availableIngredientes) {
		this.availableIngredientes = availableIngredientes;
	}
	...
	public Integer getSelectedCantidad() {
		return selectedCantidad;
	}

	public void setSelectedCantidad(Integer selectedCantidad) {
