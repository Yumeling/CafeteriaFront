...
public class PlatoBean implements Serializable {
	...
	private PlatoDTO platoSeleccionado = new PlatoDTO();
	...
	public void cargarPlatos() {
		try {
			...
			if (json.startsWith("[")) {
				java.lang.reflect.Type t = new TypeToken<List<PlatoDTO>>() {
				}.getType();
	...
	public void cargarRecetasDisponibles() {
		try {
			HttpRequest req = HttpRequest.newBuilder().GET().uri(URI.create(RECETAS_URL))
					.header("Content-Type", "application/json").build();
	...
	private IngredienteDTO fetchIngredienteByCodigo(Integer codigo) {
		if (codigo == null)
			return null;
		try {
			HttpRequest req = HttpRequest.newBuilder().GET().uri(URI.create(ING_URL + "/" + codigo))
					.header("Content-Type", "application/json").build();
			HttpResponse<String> resp = httpClient.send(req, HttpResponse.BodyHandlers.ofString());
	...
	public void onRecetaChange() {
		try {
			...
			HttpRequest req = HttpRequest.newBuilder().GET().uri(URI.create(RECETAS_URL + "/" + selectedRecetaId))
					.header("Content-Type", "application/json").build();

			HttpResponse<String> resp = httpClient.send(req, HttpResponse.BodyHandlers.ofString());
			if (!(resp.statusCode() >= 200 && resp.statusCode() < 300)) {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Aviso",
						"No se pudo recuperar la receta (status " + resp.statusCode() + ")."));
				return;
			}
			...
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error",
					"Error al leer receta o ingredientes: " + e.getMessage()));
			nuevoPlato.setCosto(null);
			nuevoPlato.setPrecioVenta(null);
			nuevoPlato.setRecetaId(selectedRecetaId);
		}
	...
	public void crearPlatoDesdeReceta() {
		try {
			...
			HttpResponse<String> resp = httpClient.send(req, HttpResponse.BodyHandlers.ofString());
			System.out.println("[PlatoBean] Response status: " + resp.statusCode() + " body: "
			...
			if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
				...
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Error", "No se pudo crear el plato. HTTP: " + resp.statusCode()));
			}
	...
	public void generarPlatosDesdeRecetas() {
		try {
			...
			for (PlatoDTO p : platos)
				if (p.getRecetaId() != null)
					conPlato.add(p.getRecetaId());

			int creados = 0;
			int saltados = 0;

			for (RecetaDTO r : recetasDisponibles) {
				if (r.getRecetaId() == null)
					continue;
				...
				if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
					...
					System.out.println("[PlatoBean BATCH] No se creó plato para receta " + r.getRecetaId() + " status: "
							+ resp.statusCode());
				}
			}

			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
					"Batch terminado", "Creados: " + creados + " | Saltados: " + saltados));
			prepararNuevoPlato();
			cargarPlatos();
	...
	public void eliminarPlato(Integer id) {
		try {
			...
			if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
				...
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Error", "No se pudo eliminar. Código: " + resp.statusCode()));
			}
			cargarPlatos();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public PlatoDTO getPlatoSeleccionado() {
		return platoSeleccionado;
	}

	public void setPlatoSeleccionado(PlatoDTO platoSeleccionado) {
		this.platoSeleccionado = platoSeleccionado;
	}

	public List<RecetaDTO> getRecetasDisponibles() {
		return recetasDisponibles;
	}

	public void setRecetasDisponibles(List<RecetaDTO> recetasDisponibles) {
		this.recetasDisponibles = recetasDisponibles;
	}

	public Integer getSelectedRecetaId() {
		return selectedRecetaId;
	}

	public void setSelectedRecetaId(Integer selectedRecetaId) {
